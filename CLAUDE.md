...
| `FROM_EMAIL`     | Lambda env var on `jaetill-portal-invite` | Postmark sender; defaults to `jason@jaetill.com`                                                                                                                                                     |
| `SIGN_IN_URL`    | Lambda env var on `jaetill-portal-invite` | "Sign in" link in nudge emails. Must be the **portal** (`https://jaetill.com/`), never the Cognito domain — `just.jaetill.com/` serves nothing at its root and 404s. Leave unset to use the default. |
...
- Cognito custom domains require an A record on the parent (`jaetill.com`) to exist _before_ `create-user-pool-domain` will accept a subdomain — chicken-and-egg with CloudFront. Create the distribution + apex A-alias first, then the Cognito domain.
